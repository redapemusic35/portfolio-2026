...
Hazlett, Alan (2019). Topics in Ethics: Desire. Washington University.;
Lindeman, Kathryn (2019). Agents and Agency. Saint Louis University.; De
Cruz, Helen (2019). Geneologies of Morality. Saint Louis University.;
Greco, John (2019). Modern Philosophy: Hume \& Reid. Saint Louis
University.
...
\tightlist
\item
  Designed and taught multiple undergraduate courses (Ethics, Logic,
  Introduction to Philosophy, Contemporary Moral Problems) as Instructor
  of Record, guiding 20--30 students per section in critical thinking,
  moral reasoning, and worldview formation.
\item
  Served two years as Assistant to the Editor for two peer-reviewed
  philosophy journals (Res Philosophica and Schutzian Research),
  managing manuscript review, editorial processes, and scholarly
  integrity.
\item
  Organized and chaired academic conferences, panels, and workshops;
  delivered 10+ papers at national and international conferences and
  chaired sessions on ethics, aesthetics, and epistemology.
\item
  Developed and led workshops on natural language processing, digital
  humanities tools, and ethical analysis for philosophers and educators,
  expanding access to advanced research methods.
\item
  Conducted digital humanities research as Graduate Research Fellow in
  the Walter J. Ong Center, building annotation tools and analyzing
  moral sentiment in texts to support ethical and interdisciplinary
  inquiry.
\item
  Mentored students, served on departmental committees, and contributed
  to graduate student initiatives, fostering collaborative and
...
\tightlist
\item
  A program for disadvantaged youth using MMOs to offer a space to
  explore art's capacity to ignite ethical reflection on our deepest
  bonds---family, friendship, love---and the meaningful paths toward
  union with God. Introduction
\end{itemize}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{data/course.phil.logic/./figures/holmes.jpg}}
...
{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{data/course.phil.logic/./figures/holmes.jpg}
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, opacitybacktitle=0.6, breakable, colframe=quarto-callout-important-color-frame, arc=.35mm, left=2mm, toptitle=1mm, bottomrule=.15mm, bottomtitle=1mm, titlerule=0mm, leftrule=.75mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{📢 Important}, rightrule=.15mm, toprule=.15mm, colbacktitle=quarto-callout-important-color!10!white, opacityback=0, coltitle=black]
...
\item
  The polls put them 20 points ahead, and a lead of 20 points with only
  a week to go to polling day can't be overturned.
\end{enumerate}
...
\tightlist
\item
  Most pelicans are corrupt.
\item
  {[}After all{]}, most ordinary birds are corrupt -- and pelicans are
  ordinary birds.
\end{enumerate}
...
\item
  Anyone who is well prepared for the race, even if she doesn't come in
  first, will at least come in the top 10.
\item
  Jane is well prepared,
\item
  {[}so{]} she will at least come in the top ten.
\end{enumerate}

Valid

\paragraph{Chapter 2}\label{chapter-2-1}
...
\item
  If an argument has false premisses and a true conclusion, then the
  truth of the conclusion can't really be owed to the premisses: so the
  argument cannot really be valid. (False)
\item
  You can make an invalid argument valid by adding extra premisses.
  (True)
\end{enumerate}

\paragraph{Chapter 3}\label{chapter-3-1}
...
\paragraph{Chapter 4}\label{chapter-4-1}
...
\tightlist
\item
  Only logicians are good philosophers.
\item
  No existentialists are logicians.
\item
  Some existentialists are French philosophers.
\item
  So, some French philosophers are not good philosophers.
\end{enumerate}
...
\tightlist
\item
  Only L are G (Premise)
\item
  No E are L (Premise)
\item
  Some E are F (Premise)
\item
  Some F are E (from 3)
\item
  Some F are not L (from 2 and 4)
\item
  So some F are not G (from 1 and 5)
\end{enumerate}
...
\item
  All E are P (Premise)
\item
  No J is L (from 2 and 3)
\item
  No J is P (from 1 and 5)
\item
  So no J is E (from 4 and 6)
\end{enumerate}

\paragraph{Chapter 6}\label{chapter-6-1}
...
\tightlist
\item
  Only L are W (Premise)
\item
  Some P are not L (Premise)
\item
  All A are W (Premise)
\item
  All A are L (from 1 and 3)
\item
  Some P are not A (from 2 and 4)
\item
  Hence some of those who don't love Aristotle are still philosophers.
  (from 5)
\end{enumerate}

\paragraph{Chapter 8}\label{chapter-8-1}
...
\item
  Peter doesn't love Jane. (\(\neg\) P)
\item
  Peter is wise and he loves Jane. (S \(\land\) P)
\item
  Either Peter loves Jane or Jesse does. (P \(\lor\) R)
\item
  Peter and Jane love each other. (P \(\land\) Q)
\item
  Neither Peter loves Jane nor does Jesse. \(\neg\) (P \(\lor\) R) or
  (\(\neg\) P \(\land\) \(\neg\) R)
\item
...
\textbf{\emph{Essays 2 (80 points each):}}
...
\textbf{\emph{D\&D Campaigns (96 points):}} Your grade in this area
...
The DM presents a summary of the assigned text. They will then present a
D\&D structured adventure of the text (this will be provided). The core
issues of the text will be narratively framed already: The group will be
a party of adventurers whose job it is to solve the issues before them
in order to advance. The reading is the ``quest'' or ``dungeon''
backdrop---e.g., a moral/ethical, metaphysical/epistemological dilemma
in a kingdom, or a riddle posed by an adversarial entity (the DM).

The DM introduces \textbf{\emph{several challenges}} sequentially. Each
challenge draws directly from key concepts, arguments, objections, or
thought experiments in the reading. Examples: You've created an
intelligent and conscious sim population \(\rightarrow\) Debate your
obligations to them. You've fallen in love with a player in an MMO who
you only know as their character \(\rightarrow\) Explore is this true
love?

For each \textbf{\emph{challenge}} any group member can respond by:
\textbf{\emph{Asking a clarifying question}} about the challenge
(probing the text/concept). \textbf{\emph{Offering a
suggestion/solution}} (proposing a philosophical position, argument, or
counter). The DM evaluates the response (as the ``world'' reacting). The
responding player \textbf{\emph{rolls dice}} (typically a d20, we will
ignore modifiers this time around) to determine whether they defend the
position or attack it.
...
After the first response/roll: Each person's response will be based on
whether they win or loose the dice roll. Other group member get turns to
\textbf{\emph{offer their own responses}} or build on/counter the
previous one. They also \textbf{\emph{roll dice}} to suggest ``other
considerations'' (e.g., other defenses of the DM's positions; other
criticism's of the DM's position; edge cases; objections from the
reading). This creates a back-and-forth discussion, with dice adding
chance, excitement, and stakes---mirroring how philosophical debates
involve risk, uncertainty, and persuasion.

Resolution \& Reflection occurs when the DM adjudicates the collective
outcomes (what is plausible?). The session ends with debrief: How did
the ``adventure'' illuminate the reading? What arguments won/lost? How
does this apply to real philosophy? Did you change your original
position?
...
\subsection{Overall Assignment Setup}\label{overall-assignment-setup}

\begin{itemize}
\tightlist
\item
  \textbf{\emph{Group Structure:}} By second week, students form small
  groups of \textbf{\emph{5 members}} each. This will be done using a
  randomizing method.
\item
  \textbf{\emph{Rotating Dungeon Master (DM) Role:}} Every group member
  gets \textbf{\emph{one turn}} to act as the Dungeon Master (DM) for a
  full session/day.

  \begin{itemize}
  \tightlist
  \item
    This distributes the workload fairly---no one is stuck doing all the
    prep.
  \item
    The DM takes ownership of that day's reading/topic. A D\&D-style
    adventure will be provided for the class and the DM will be given
    the adventure. They then give the problems to the other students in
    a turn-based format. Ideally you begin by imagining being in a
    unique situation that makes you question some belief that you've
    always taken for granted, an example will be provided which you may
    use instead of creating your own.
  \item
    The DM will role a d20 dice first to determine what positions the
    students will take.
  \end{itemize}
\end{itemize}

\subsection{Play Through}\label{play-through}

The D\&D assignment is not exactly like D\&D though Ive tried to keep it
as close as possible. The idea is to encourage students to talk through
the philosophical issues we read about.

Ideally you begin by imagining being in a unique situation that makes
you question some belief that you've always taken for granted.

Each day that we do the activity, each group will have a new DM for that
day. The DM rolls the dice to set the difficulty class. They then read
the encounter.
...
\item
  Therefore there will be billions of \emph{sim} people in the sim
  universe.
\item
  Such a program, the \emph{simUniverse}, will likely be very successful
  and there will be many, many \emph{simUniverses} sold.
\end{itemize}

Therefore there will be many more sims than non sims.

\subsection{Encounter}\label{encounter}

The DM then reads an imagined context that motivates each of the reasons
above in a turn based format.

Going around the table, the DM has each member of their group roll a d20
to determine what position on one of the reasons above, they are going
to take. Do they defend the reason given, or do they critique it?

First, as a class we take three polls, one for each reason. Is there
more who agree with the reason above, or disagree with it?
...
\tightlist
\item
  If the other student rolls higher, they get to defend the intuitive
  position.
\item
...
\tightlist
\item
  If the other student rolls lower than the DM, then they will have to
  defend the least likely answer.
\item
  The position with the least agreement for.
\end{itemize}

The students gives their position which is recorded onto everyone's
tally sheets. At the end of the campaign for that day, each sheet should
have a set of positions for each of the reasons above, either defending
it, or objecting to it, one from each member of the group.

Everyone should have their own copy of the tally sheet. You will upload
a photo of it to Canvas under the assignment module. You keep the tally
...
\pandocbounded{\includegraphics[keepaspectratio]{data/course.phil.law/./../figures/professor_octopus.png}}\hfill
